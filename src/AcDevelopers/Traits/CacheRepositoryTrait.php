...
namespace AcDevelopers\EloquentRepository\Traits;
 ...
 *
 * @package AcDevelopers\EloquentRepository\Traits
 */
trait CacheRepositoryTrait
{
    /**
    ...
    public function getCacheRepository()
    {
        if (is_null($this->cacheRepository)) {
            $this->cacheRepository = app(config('ac-developers.repository.cache.repository', 'cache'));
        }
    ...
    public function isSkippedCache()
    {
        ...
        $skipCacheParam = config('ac-developers.repository.cache.params.skipCache', 'skipCache');
    ...
    {
        $cacheEnabled = config('ac-developers.repository.cache.enabled', true);

        if (!$cacheEnabled) {
            return false;
        }

        $cacheOnly = isset($this->cacheOnly) ? $this->cacheOnly : config('ac-developers.repository.cache.allowed.only', null);
        $cacheExcept = isset($this->cacheExcept) ? $this->cacheExcept : config('ac-developers.repository.cache.allowed.except', null);
    ...
    {
        $cacheMinutes = isset($this->cacheMinutes) ? $this->cacheMinutes : config('ac-developers.repository.cache.minutes', 30);
